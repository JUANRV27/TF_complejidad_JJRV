...
#El segundo mejor código hasta ahora
...
def update_graph(start_node, end_node, toggle_value, relayout_data):
    # Si el cambio en el mapa solo es por zoom o desplazamiento, no actualizamos el camino más corto
    if start_node is None or end_node is None:
        # Si no hay nodos seleccionados, no actualizamos el camino más corto
        fig_path = go.Figure(
            data=[edges_trace],  # Solo mostrar las aristas sin el camino más corto
            layout=go.Layout(
        ...
        # Si ambos nodos están seleccionados, calculamos el camino más corto
        try:
    ...
    # Mapa principal (se actualizará si se selecciona el toggle para mostrar el grafo completo)
    zoom_level = relayout_data.get('mapbox.zoom', 10)
    
    data_main = [edges_trace, main_nodes_trace]
    if 'show_graph' in toggle_value:  # Solo se agrega el grafo si el toggle está seleccionado
        data_main.append(secondary_nodes_trace)
